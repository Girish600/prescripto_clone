...
const DoctorProfile = () => {
  ...
  const {doctorToken, profileData,backendUrl , setProfileData, getProfileData} = useContext(DoctorContext);
  ...
  const updateProfile= async()=>{
    try {
      ...
      const {data}= await axios.post(backendUrl + '/doctor/update-profile', updateData, {headers:{Authorization:`Bearer ${doctorToken}`}});
  ...
  return profileData && (
    <div>
      <div className='flex flex-col gap-4 m-5'>
        <div>
          <img className='bg-[#5f6FFF]/80 w-full sm:max-w-64 rounded-lg' src={profileData.image}/>
        </div>
